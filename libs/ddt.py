...
# For the exact contribution history, see the git revision logs.
...
DATA_ATTR = '%values'      # store the datas the test must run with
...
def file_data(value):
    ...
    should contain JSON encoded datas, that can either be a list or a
    dict.
    ...
    test case, and values will be fed as test datas.
...
def mk_test_name(name, value, index=0):
    ...
    #     return "{0}_{1}".format(name, index)
    #如果数据是字典，则获取字典当中的title对应的值，加到测试用例的名称中
...
def feed_data(func, new_name, test_data_docstring, *args, **kwargs):
    """
    This internal method decorator feeds the test datas item to the test.
...
def process_file_data(cls, name, func, file_attr):
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        # Load the datas from YAML or JSON
        if _is_yaml_file:
...
def _add_tests_from_data(cls, name, func, data):
    """
    Add tests from datas loaded from the datas file into the class
    """
...
def ddt(cls):
    ...
    decorate test methods with ``@datas``.

    For each method decorated with ``@datas``, this will effectively create as
    many methods as datas items are passed as parameters to ``@datas``.

    The names of the test methods follow the pattern
    ``original_test_name_{ordinal}_{datas}``. ``ordinal`` is the position of the
    datas argument, starting with 1.

    For datas we use a string representation of the datas value converted into a
    valid python identifier.  If ``datas.__name__`` exists, we use that instead.
